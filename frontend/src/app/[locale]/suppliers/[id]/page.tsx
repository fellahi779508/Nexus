...
import styles from "./detailedSupplier.module.css";
import {
  deleteCreditsofSupplier,
  GetSupplierById,
  updateSupplier,
...
export default function DetailedSupplier() {
  const t = useTranslations("DetailedSupplier");
  ...
  const fetchSupplier = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 1) {
        ...
        setError(res.response?.toString() || t("errorFetch"));
      }
    } catch (err) {
  ...
  const handleRecordPayment = async () => {
    let newTTC = supplier?.creditTTC! - globalPayment;
    if (newTTC < 0) {
      newTTC = 0;
    }
    const res = await updateSupplier(id, { creditTTC: newTTC });
    if (res.status === 1) {
      setGlobalPayment(0);
      fetchSupplier();
  ...
  const handleMarkAllAsPaid = async () => {
    const res = await deleteCreditsofSupplier(id);
    if (res.status === 1) {
      fetchSupplier();
  ...
  return (
    <div className={styles.container}>
      <title>Nexus | Supplier Details</title>
      <div className={styles.header}>
        <div>
          <h1 className={styles.title}>{t("title", { id: supplier.id })}</h1>
          <span className={styles.dateBadge}>
            {t("supplierSince", {
              date: new Date(supplier.createdAt).toLocaleDateString(undefined, {
                year: "numeric",
                month: "long",
                day: "numeric",
                hour: "2-digit",
      ...
      <div className={styles.grid}>
        <section className={styles.card}>
          ...
          <h2 className={styles.sectionTitle}>{t("supplierInfo")}</h2>
        ...
        <section className={styles.card}>
          ...
          <div className={styles.creditValue}>
            ...
            <span
              className={`${styles.valueHighlight} ${supplier.creditTTC > 0 ? styles.textDanger : styles.textSuccess}`}
            >
              {supplier.creditTTC.toFixed(2)} {t("currency")}
            </span>
          </div>
      ...
      <section className={styles.card}>
        <div className={styles.cardAccent} />
        <h2 className={styles.sectionTitle}>{t("purchases")}</h2>
        {!supplier.stockPayments || supplier.stockPayments.length === 0 ? (
          <p className={styles.empty}>{t("noPurchases")}</p>
        ) : (
          <div className={styles.tableWrapper}>
            <table className={styles.table}>
              ...
              <tbody>
                {supplier.stockPayments?.map((sale) => {
